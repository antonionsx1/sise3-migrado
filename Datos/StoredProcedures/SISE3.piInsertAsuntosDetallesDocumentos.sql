...
-- Original : [dbo].[usp_AsuntosDetallesDocumentosIns]
-- ============================================= 
ALTER PROCEDURE [SISE3].[piInsertAsuntosDetallesDocumentos]
 @pi_AsuntoNeunId INT						
,@pi_PersonaId INT						
,@pi_EmpleadoId INT
...
AS
	BEGIN
		...
		BEGIN TRY
			...
			DECLARE @ResultadoExpediente BIGINT = 58
			...
			IF NOT EXISTS(SELECT CatTipoAsuntoId FROM CamposPropiedades WITH(NOLOCK) 
							WHERE TipoPropiedadId = 28 AND StatusReg = 1 AND CatTipoAsuntoId = @pi_CatTipoAsuntoId)
			BEGIN
			...
			IF NOT EXISTS(SELECT CatTipoAsuntoId FROM CamposPropiedades WITH(NOLOCK)
				...
				BEGIN
					...
					BEGIN		
						...
						VALUES (@pi_CatOrganismoId, @pi_AsuntoNeunId, @pi_PersonaId, @NoBloque, @FechaRegistro ,1, @pi_EmpleadoId)
						SET @DocumentoId = (SELECT ISNULL(MAX(DocumentoId),1) FROM AsuntosApartadoDocumentos)						
					END
					...
					SET @DatosXML = (SELECT [dbo].[fn_ObtieneDatosDocumentosXMLBilletes](@pi_AsuntoNeunId, @pi_PersonaId, @NoBloque))
					
					If @DatosXML IS NOT NULL
					BEGIN
						EXEC [SISE_NEWLOG].[dbo].[usp_BitacoraAsuntoDocumentos] @pi_AsuntoNeunId, @pi_PersonaId, @NoBloque ,1, @pi_EmpleadoId, @DatosXML
					END
				END
			ELSE			
				BEGIN				
					EXEC [SISE3].[piInsertAsuntosDetallesDocumentos_SinPartes] @pi_AsuntoNeunId, @pi_EmpleadoId, @pi_AsuntoDetalleCatalogos_type, @pi_AsuntoDetalleDescripcion_type, @pi_AsuntoDetalleFechas_type								
				END
